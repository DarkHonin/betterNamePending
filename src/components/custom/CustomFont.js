...
class CustomFont extends Custom{
    constructor(props){
        super(props)
    ...
    update_font(e){
        ...
        this.props.onChange(this.child.tag, `font-family`, font)
    ...
    componentDidMount(){
        ...
        let getFontThings = (() => {
            const font_options = this.child.fonts.map((value, index) => <option className={`font_${index}`} key={index} value={index}>{value}</option>)
            const font_refrence = this.child.fonts.map((v) => v)
